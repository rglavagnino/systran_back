...
export class RaiddController {
  constructor(private readonly raidSrv: RaiddService) {}
  ...
  async insertarElementoBitacora(
    ...
    try {
      if (token === 'DTSI IS GAH') {
        const resul = await this.raidSrv.insertarElemento(
          obs,
          tipo,
          etiqueta,
          usuario,
        ...
        const sal = crearSalida(
          'No esta autorizado para ingresar una bitacora',
          obtenerTipo(3),
          '',
          [],
        );
