...
export class RaiddService {
    constructor(
        @InjectModel('ElementoBitacora') private readonly elementoBitacoraModel: Model<elementoBitacora>
        ,@InjectModel('EstadoBitacora') private readonly estadoBitacoraModel: Model<estadobitacora>
    ){
    ...
    separarEtiquetas(listadoEtiqueta:string):msgInterno{
        ...
        let mensaje:msgInterno = { tipo : 0, mensaje:'', dataUnico:1,dataArreglo:[]}
        if (listadoEtiqueta){
     ...
     * id:1001
     * ANCHOR buscar estado mas nuevo
     */
    async buscarEstadoMasNuevo(){
        let resul:msgInterno = { tipo : 0, mensaje:'', dataUnico:1,dataArreglo:[]}
        logger('1001 - Buscando el estado mas nuevo')
        const primerEstado = await this.estadoBitacoraModel.findOne(
            {activo:1}
            ,{_id:0,__v:0,activo:0}
            ,{oid:1}
        ).catch((err=> {
            resul.tipo = 0
            resul.mensaje = err
...
async actualizarObservacion(
    ...
    if (!usuario){
        loggerId(usuario,'Actualización fallida de etiqueta, falta el usuario creador ',idFuncion )
        const sal = crearSalida('Actualización fallida de etiqueta, falta el usuario creador ' , obtenerTipo(3),'Falta el usuario',[])
        return sal
    }
    ...
    if (mongoose.isValidObjectId(id) == false){
        loggerId(usuario,'Falla la actualización debido a que no se tiene un id valido',idFuncion)
        const salida = crearSalida('Falla la actualización debido a que no se tiene un id valido',obtenerTipo(3),'',[])
        return salida
    }
    loggerId(usuario,'Actualización la etiqueta '+ this.eliminarEtiqueta + ' ' + id,idFuncion)
    ...
    }else 
    {
        ...
        loggerId(usuario,'Observacion encontrada ' + obsEncontrada.observaciones,idFuncion)
 ...
 * @returns 
 */
    async insertarElemento(observaciones:string, tipo:number, etiqueta:string, usuario:string){
    
        if (!usuario){
            const sal = crearSalida('Creacion fallida de elemento bitacora, falta el usuario creador ' , obtenerTipo(3),'Falta el usuario',[])
        ...
        if (msgEstadoInsertar.tipo === 0 ){
            const sal = crearSalida('Creacion fallida de elemento bitacora ' + tipo, obtenerTipo(3),msgEstadoInsertar.mensaje,[])
        ...
        if (tipo)
            ...
            const sal = crearSalida('Creacion fallida de elemento bitacora, falta el tipo ' , obtenerTipo(3),'Falta el tipo',[])
        ...
        await elementoNuevo.save().catch((err)=> {
            const sal = crearSalida('Creacion fallida de elemento bitacora ' + tipo, obtenerTipo(3),err,[])
            logger('1002 - Creacion fallida de elemento bitacora ' + tipo)
            return sal
        })
        let resul = [];
        resul.push(elementoNuevo)
        const sal = crearSalida('Creacion exitosa de elemento bitacora ' + tipo,obtenerTipo(2),'',resul)
 ...
 */
    async obtenerTodosElementosBitacora(usuario:string){
        ...
        loggerId(usuario,'obteniendo todos los elementos de la bitacora',idFuncion)
        ...
        loggerId(usuario,'Se obtuvieron ' + arr.length.toString() + ' resultados',idFuncion)
 ...
 */
    async eliminarEtiqueta(
        ...
        if (!usuario){
            loggerId(usuario,'Eliminacion fallida de etiqueta, falta el usuario creador ',idFuncion )
            const sal = crearSalida('Eliminacion fallida de etiqueta, falta el usuario creador ' , obtenerTipo(3),'Falta el usuario',[])
            return sal
        }
        ...
        if (mongoose.isValidObjectId(id) == false){
            logger('1005 - Falla la eliminacion debido a que no se tiene un id valido')
            const salida = crearSalida('Falla la eliminacion debido a que no se tiene un id valido',obtenerTipo(3),'',[])
            return salida
        }
 ...
 */
     async insertarEtiqueta(
        ...
        if (!usuario){
            loggerId(usuario,'Agregación fallida de etiqueta, falta el usuario creador ',idFuncion )
            const sal = crearSalida('Agregación fallida de etiqueta, falta el usuario creador ' , obtenerTipo(3),'Falta el usuario',[])
            return sal
        }
        if (!nuevaEtiqueta){
            const msg = 'Falla la eliminacion debido a que no se tiene una etiqueta valida'
        ...
        loggerId(usuario,'Agregando la etiqueta ' + nuevaEtiqueta + ' en el tarea ' + id,idFuncion)
        
        if (mongoose.isValidObjectId(id) == false){
            loggerId(usuario ,'Falla la eliminacion debido a que no se tiene un id valido',idFuncion)
            const salida = crearSalida('Falla la eliminacion debido a que no se tiene un id valido',obtenerTipo(3),'',[])
            return salida
        }
        ...
        }else 
        {
            ...
            loggerId(usuario,'Buscando la etiqueta ' + nuevaEtiqueta + ' para revisar si existe', idFuncion)
            let el = etiquetas.findIndex((x) => {
