...
export class CatService {
    constructor(
        @InjectModel('Tipo') private readonly tipoBitacoraModel:Model<tipobitacora>
        ,@InjectModel('Estado') private readonly estadoBitacoraModel:Model<estadobitacora>
    ){}
    ...
    async crearCatalogo(){
        ...
        if (!tipoJson){ //existe datos en los archivs
            let msg = 'No hay data en los catalogos'
            logger(msg)
            resultado = crearSalida(
                msg,obtenerTipo(3),'',[])
